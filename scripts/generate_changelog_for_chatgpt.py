...
def ml_rework_stats(df, means):
    """Tablica prije/poslije i broj pobjeda nad linearnom interpolacijom."""
    lin = df[df.method == "linear_interpolation"].set_index(["scenario", "missing_rate"]).mae

    rows = []
    for method, before in ML_BEFORE.items():
        after = float(means[method])
        rows.append(
    ...
    win_rows = []
    for method in ["neural_net", "random_forest", "decision_tree", "knn_upgraded", "knn",
                   "cubic_interpolation", "moving_average", "forward_fill"]:
...
def build_thesis_doc(df, means, knn_df, knn_basic, knn_upg, ma_mean, linear_mean, adaptive_mean):
    ...
    before_after_table, wins_table = ml_rework_stats(df, means)
...
|--------|-------------------|-------------|----------|
{before_after_table}
| `neural_net` | — (nova metoda) | {nn_mean:.4f} | — |

### C.4 Broj pobjeda nad linearnom interpolacijom

Usporedba po pojedinačnim kombinacijama scenarij × missing rate (referenca: `linear_interpolation`, MAE = {linear_mean:.4f} °C):

| Metoda | Prosječni MAE | Pobjeda nad linear |
|--------|---------------|--------------------|
{wins_table}

Neuronska mreža ima najniži prosječni MAE među svim metodama koje ne koriste oracle routing (**{nn_mean:.4f}** naspram **{linear_mean:.4f}** °C za linearnu interpolaciju).
...
**Zaključak:** nakon prerade napredni KNN postiže niži prosječni MAE (**{knn_upg:.4f}** vs **{knn_basic:.4f}** °C). U prvoj verziji odnos je bio obrnut (4,9931 vs 3,7386) jer je „napredna” varijanta zbog pogrešnog omjera težina tražila susjede po **dobu dana** umjesto po **blizini u nizu**.
...
Nakon uvođenja zajedničkog prostora značajki i učenja reziduala, **četiri ML metode nadmašuju linearnu interpolaciju** u prosječnom MAE: neuronska mreža (**{nn_mean:.4f}**), slučajna šuma (**{rf_mean:.4f}**), stablo odlučivanja (**{dt_mean:.4f}**) i napredni KNN (**{knn_upg:.4f}**), naspram **{linear_mean:.4f} °C** za linearnu interpolaciju.
...
`adaptive_imputation` postiže najniži prosječni MAE ({adaptive_mean:.4f} °C), ali njegova routing tablica u `src/adaptive_imputation.c` ručno je popunjena metodama koje su pobijedile **na istom test skupu** na kojem se metoda ocjenjuje. Broj slobodnih parametara jednak je broju testova, pa je riječ o savršenom prenaučenju po konstrukciji, bez ikakve generalizacije.

Rezultat zato treba prikazati kao **oracle granicu** — „najbolje što bi se postiglo savršenim odabirom metode po scenariju” — a ne kao rezultat metode. Tako protumačen postaje koristan: razlika između linearne interpolacije ({linear_mean:.4f}) i oracle granice ({adaptive_mean:.4f}) iznosi oko {(1 - adaptive_mean / linear_mean) * 100:.0f} %, i to je **cjelokupan mogući dobitak** od bilo kakvog pametnog odabira metode. Neuronska mreža od tog raspona uzima približno {(linear_mean - nn_mean) / (linear_mean - adaptive_mean) * 100:.0f} %.
...
2. **Neuronska mreža** ima najniži prosječni MAE među metodama bez oracle routinga (**{nn_mean:.4f} °C** naspram **{linear_mean:.4f} °C** za linearnu interpolaciju).
...
4. **Linear interpolacija** ostaje nenadmašena na kratkim prazninama ({linear_wins} pobjeda po MAE) jer je za lokalno linearan signal analitički optimalna; ML metode je nadmašuju na dugim prazninama.
...
6. **Adaptivna imputacija** ({adaptive_mean:.4f} °C) treba se tumačiti kao **oracle granica**, jer joj je routing tablica popunjena rezultatima s test skupa.
...
| `src/experiment.c` | {n_methods} metoda, export najbolje/najgore rekonstrukcije |
